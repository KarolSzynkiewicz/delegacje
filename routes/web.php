...
Route::middleware(['auth', 'verified', 'role.required', 'permission.check'])->group(function () {
    ...
    Route::group(['defaults' => ['permission_type' => 'action']], function () {
        ...
        Route::post('/system-actions/sync-work-items', [SystemActionsController::class, 'syncWorkItems'])
    ...
    Route::group(['defaults' => ['permission_type' => 'resource']], function () {
        ...
        Route::post('comments/{comment}/like', [\App\Http\Controllers\CommentController::class, 'toggleLike'])
            ->name('comments.like');
        Route::post('comments/{comment}/pin', [\App\Http\Controllers\CommentController::class, 'togglePin'])
            ->name('comments.pin');
        Route::post('comments/{comment}/mention-task', [\App\Http\Controllers\CommentController::class, 'toggleMentionTask'])
            ->name('comments.mention-task.toggle');
        ...
        Route::get('warehouse-dispatches/{warehouseDispatch}', [\App\Http\Controllers\EquipmentIssueController::class, 'showDispatch'])
            ->name('warehouse-dispatches.show')
            ->defaults('resource', 'equipment-issues');
