...
class ProjectDemandController extends Controller
{
    ...
    {
        $query = ProjectDemand::query()
            ->with('project', 'role')
            ->where('required_count', '>', 0);

        $searchProject = $request->string('searchProject')->trim()->toString();
        if ($searchProject !== '') {
            $query->whereHas('project', function ($q) use ($searchProject) {
                $q->where('name', 'like', '%'.addcslashes($searchProject, '%_\\').'%');
            });
        }
        ...
        return view('demands.all', compact('demands', 'roles'));
    }
