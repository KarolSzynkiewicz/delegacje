@php
    ...
    $canDecide = auth()->user()?->can('decide', $approval) ?? false;
...
@if($canDecide && ! $approval->isDecided())
    <x-ui.card class="mb-4">
        ...
        @if($embedded)
            <label class="form-label small text-muted" for="plan-approval-comment">Uzasadnienie (opcjonalnie)</label>
            <textarea id="plan-approval-comment"
                      class="form-control mb-3"
                      rows="3"
                      maxlength="5000"
                      wire:model="approvalComment"
                      placeholder="Dlaczego zatwierdzasz albo odrzucasz?"></textarea>
            @error('approvalComment')
                <div class="invalid-feedback d-block mb-2">{{ $message }}</div>
            @enderror
            <div class="d-flex flex-wrap gap-2">
                <x-ui.button variant="success" type="button" wire:click="decideOpenApproval('approved')">
                    <i class="bi bi-check-lg me-1"></i>Zatwierdź
                </x-ui.button>
                <x-ui.button variant="danger" type="button" wire:click="decideOpenApproval('rejected')">
...
@if($showComments)
    <x-comments :commentable="$approval" :embedded="$embedded" />
@endif
