<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Wydania z magazynu">
            <x-slot name="right">
                @include('equipment._warehouse-switcher', [
                    'warehouses' => $warehouses,
                    'current' => $warehouse,
                    'routeName' => 'equipment-issues.index',
                    'keep' => request()->only(['status', 'employee_id', 'equipment_id']),
                ])
                <x-ui.button
                    variant="primary"
    ...
    <x-ui.card class="mb-4">
        ...
        @if(request('status') || request('employee_id') || request('equipment_id'))
            <div class="mt-3 pt-3 border-top">
                <a href="{{ route('equipment-issues.index', ['warehouse_id' => $warehouse->id]) }}" class="btn btn-sm btn-outline-secondary">
    ...
    <x-ui.card>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        @if($issue->status === 'issued' && $issue->equipment->issuable && $issue->equipment->returnable)
        ...
        @else
            <x-ui.empty-state
                icon="inbox"
                :message="request('status') || request('employee_id') || request('equipment_id') ? 'Brak wydań spełniających kryteria wyszukiwania' : 'Brak wydań z magazynu.'"
            >
                @if(!request('status') && !request('employee_id') && !request('equipment_id'))
                    <x-ui.button
                        variant="primary"
                ...
                @else
                    <a href="{{ route('equipment-issues.index', ['warehouse_id' => $warehouse->id]) }}" class="btn btn-outline-secondary">
                        <i class="bi bi-x-circle me-1"></i> Wyczyść filtry
                    </a>
                @endif
            </x-ui.empty-state>
