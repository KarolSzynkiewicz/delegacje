<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Edytuj Zaliczkę">
            <x-slot name="left">
                <x-ui.button
                    variant="ghost"
                    ...
                    <div class="mb-3">
                        ...
                        >
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" {{ (string) old('employee_id', $advance->employee_id) === (string) $employee->id ? 'selected' : '' }}>
                                    {{ $employee->full_name }}
                                </option>
                            @endforeach
                        </x-ui.input>
                        <small class="form-text text-muted">Pracownik, którego dotyczy zaliczka</small>
                    </div>
                        ...
                        >
                            ...
                            @foreach($payrolls as $payroll)
                                <option
                                    ...
                                    {{ old('payroll_id', $advance->payroll_id) == $payroll->id ? 'selected' : '' }}
                                >
                        ...
                        <div class="col-md-6">
                            ...
                            >
                                @foreach(\App\Enums\Currency::cases() as $c)
                                    <option value="{{ $c->value }}" {{ old('currency', $advance->currency) == $c->value ? 'selected' : '' }}>
                                        {{ $c->label() }}
                                    </option>
                                @endforeach
                            </x-ui.input>
