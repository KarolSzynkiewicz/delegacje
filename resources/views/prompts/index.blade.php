...
<x-app-layout>
    ...
    <div
        ...
        data-export-time-logs-url="{{ route('prompts.export.time-logs') }}"
    ...
    >
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-3">
            ...
            <div class="col d-flex">
                <x-ui.card variant="hover" class="w-100 d-flex flex-column">
                    <div class="d-flex align-items-start gap-2 mb-2">
                        <span class="rounded-2 p-2 bg-info bg-opacity-10 text-info"><i class="bi bi-clock-history fs-5"></i></span>
                        <div class="min-w-0">
                            <div class="fw-semibold">Godziny pracy (raw JSON)</div>
                            <div class="text-muted small">Wpisy czasu pracy per pracownik, projekt i spółka — dla zakresu dat.</div>
                        </div>
                    </div>
                    <div class="mt-auto pt-2">
                        <x-ui.button
                            ...
                            type="button"
                            data-bs-toggle="modal"
                            data-bs-target="#promptEngineTimeLogsModal"
                        >
                            Wybierz zakres i pobierz
                        </x-ui.button>
                    </div>
    ...
    >
        ...
        <div
            class="modal fade departure-planner-teleport-modal"
            id="promptEngineTimeLogsModal"
            tabindex="-1"
            aria-labelledby="promptEngineTimeLogsModalLabel"
            aria-hidden="true"
        >
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content border-secondary" style="background: var(--bg-card, #1e293b); color: var(--bs-body-color, #e2e8f0);">
                    <div class="modal-header border-secondary">
                        <h5 class="modal-title" id="promptEngineTimeLogsModalLabel">
                            <i class="bi bi-clock-history me-2 text-info"></i>Eksport godzin pracy (JSON)
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Zamknij"></button>
                    </div>
                    <div class="modal-body">
                        <div id="prompt-engine-timelogs-error" class="alert alert-danger py-2 small d-none" role="alert"></div>
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <x-ui.input type="date" name="ptl_start_date" id="ptl_start_date" label="Data od" :value="$defaultStart" required />
                            </div>
                            <div class="col-md-6">
                                <x-ui.input type="date" name="ptl_end_date" id="ptl_end_date" label="Data do" :value="$defaultEnd" required />
                            </div>
                        </div>
                        <div class="d-flex flex-wrap gap-2 mb-3">
                            <x-ui.button variant="info" type="button" id="ptl-fetch">Generuj JSON</x-ui.button>
                            <x-ui.button variant="ghost" type="button" id="ptl-copy" disabled>Skopiuj do schowka</x-ui.button>
                        </div>
                        <div id="ptl-loading" class="text-muted small d-none mb-2">
                            <span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
                            Ładowanie…
                        </div>
                        <label class="form-label small text-muted" for="ptl-json">Wynik</label>
                        <textarea
                            id="ptl-json"
                            class="form-control font-monospace small"
                            rows="14"
    ...
    @push('scripts')
        <script>
            ...
            (function () {
                ...
                const modalEl = document.getElementById('promptEngineTimeLogsModal');
                if (modalEl && modalEl.parentElement !== document.body) {
                    document.body.appendChild(modalEl);
                }

                const exportUrl = root.dataset.exportTimeLogsUrl;
                const defaultStart = root.dataset.defaultStart;
                const defaultEnd = root.dataset.defaultEnd;
                const startInput = document.getElementById('ptl_start_date');
                const endInput = document.getElementById('ptl_end_date');
                const fetchBtn = document.getElementById('ptl-fetch');
                const copyBtn = document.getElementById('ptl-copy');
                const textarea = document.getElementById('ptl-json');
                const loadingEl = document.getElementById('ptl-loading');
                const errorEl = document.getElementById('prompt-engine-timelogs-error');
