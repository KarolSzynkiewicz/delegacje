<div>
    <x-data-table :paginator="$transfers" :has-filters="$this->hasLogisticsFilters()">
        <x-slot:filters>
        ...
        <x-slot:body>
            @foreach ($transfers as $transfer)
                @include('livewire.partials.transfers-row', ['transfer' => $transfer])
            @endforeach
        </x-slot:body>
        <x-slot:cards>
            @foreach ($transfers as $transfer)
                @include('livewire.partials.transfers-row-card', ['transfer' => $transfer])
            @endforeach
        </x-slot:cards>
        <x-slot:empty>
            <x-ui.empty-state icon="arrow-left-right" message="Brak transferów" :has-filters="$this->hasLogisticsFilters()" clear-filters-action="wire:clearFilters" />
        </x-slot:empty>
    </x-data-table>
</div>
