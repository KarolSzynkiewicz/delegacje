...
<x-ui.card label="Historia">
    <p class="small text-muted mb-3">
        Kto i kiedy dodawał, kończył, komentował, przenosił i przypisywał zadania oraz podzadania w tym sprincie.
    </p>

    @forelse($entries as $entry)
        @php $color = $tone[$entry['tone']] ?? $tone['muted']; @endphp
        <div
            wire:key="sprint-act-{{ $entry['kind'] }}-{{ $entry['at']->timestamp }}-{{ $loop->index }}"
    ...
    @empty
        <p class="text-muted mb-0">Brak zdarzeń w tym sprincie.</p>
    @endforelse
</x-ui.card>
