...
<div {{ $attributes->merge(['class' => 'rounded-3 p-2 transition-all d-flex flex-column w-100 h-100 logistics-trip-header-card' . ($missing ? ' logistics-trip-header-card--invalid' : '')]) }}
    ...
    <div class="d-flex gap-2 align-items-stretch logistics-trip-header-control-row" style="overflow: hidden;">
        <button type="button"
                ...
                @endif
                class="btn flex-fill d-inline-flex align-items-center justify-content-center logistics-trip-header-control min-w-0 {{ $mode === 'public' ? 'btn-primary' : 'btn-outline-secondary' }}{{ $interactive ? '' : ' pe-none' }}"
                style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                @if(! $interactive) title="Podgląd — bez edycji" @endif>
        ...
        <button type="button"
                ...
                class="btn flex-fill d-inline-flex align-items-center justify-content-center logistics-trip-header-control min-w-0 {{ $mode === 'own' ? 'btn-success' : 'btn-outline-secondary' }}{{ $interactive ? '' : ' pe-none' }}"
                style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                @if(! $interactive) title="Podgląd — bez edycji" @endif>
