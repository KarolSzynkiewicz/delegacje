    ...
    <div class="col-lg-8">
        <x-ui.card>
            @if($story)
                ...
                @if($story['participants']->isNotEmpty())
                    <div class="mb-3">
                        <div class="small text-muted mb-2">Uczestnicy</div>
                        <div class="d-flex flex-column gap-2">
                            @foreach($story['participants'] as $person)
                                <x-ui.person :user="$person" avatar-size="28px" :show-email="false" />
                            @endforeach
                        </div>
                    </div>
                @endif
                ...
                @if($story['note'] !== '')
                    <div class="mb-1 small text-muted">Opis:</div>
                    <blockquote class="mb-0 px-3 py-2 rounded-3" style="background: rgba(59, 130, 246, 0.08); border-left: 3px solid var(--primary); white-space:pre-wrap">{{ $story['note'] }}</blockquote>
                @endif
            @else
